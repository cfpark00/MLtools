...
class PatchAugmentDataset(torch.utils.data.Dataset):
    def __init__(self,file_path,n_samples,patch_size=256):
        ...
        if "W" not in self.h5.attrs.keys() or "H" not in self.h5.attrs.keys():
            raise ValueError("W and H must be in the attributes of the h5 file, different sizes not implemented")
        ...
        with tqdm.tqdm(total=self.N_images) as pbar:
            for i in range(self.N_images):
                im,mask=np.array(self.h5[str(i)+"/im"])[0,:,:,0],np.array(self.h5[str(i)+"/mask"])[0,:,:,0]
                im_clahe=clahe(im)
    ...
    def __getitem__(self,i):
        ...
        return torch.from_numpy(im_/np.iinfo(self.im_dtype).max)[None].to(dtype=torch.float32),torch.from_numpy(mask_/np.iinfo(self.mask_dtype).max).to(dtype=torch.int64)
