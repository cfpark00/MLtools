...
def color_mask(mask,rgba_float=[1.,0.,1.,0.5]):
...
def get_prob(image,net,return_dtype=np.uint8):
    ...
    with torch.no_grad():
        mask_logits=net(res.to(device=next(net.parameters()).device,dtype=torch.float32))
...
def get_best_models(models_folpath,top=3):
    ...
    for model_path in model_paths:
        valaccs.append(float(os.path.split(model_path)[-1].split("_")[-1].split(".pth")[0]))
...
def import_matlab(lib_fol):#"/n/home12/cfpark00/matlabpython/lib/"
    ...
    """
    warning.warn("This function will create two global variables: matlab and eng,\
...
def watershed_matlab(im):
    return np.array(eng.watershed(matlab.uint8(im),8.))
...
def get_error_map(fm_prob,sm_prob,only_vi=False,return_all=False,use_matlab=False,rm_bounds=5,return_time=False,max_size=200000):
    ...
    if rm_bounds>0:
        ...
        not_boundary=np.logical_and(skmorph.binary_erosion(fm_wshed>0,ker),sm_prob<mb_thres_low)
    ...
    if only_vi:
        if return_time:
            return vi,vi_split,vi_merge,splitters,mergers,fm_wshed,sm_wshed,[t0,t1,t2,t3,t4,t5]
        else:
    ...
    for i_split in i_splits:
        error_map[(sm_wshed==i_split)*(sm_hmin<mb_thres)*(fm_hmin>mb_thres)]=True
    for i_merge in i_merges:
        error_map[(fm_wshed==i_merge)*(sm_hmin>mb_thres)*(fm_hmin<mb_thres)]=True
    for miss in misses:
        error_map[sm_wshed==miss]=True
    for extra in extras:
        error_map[fm_wshed==extra]=True

    error_map=skmorph.binary_dilation(error_map,np.ones((3,3))).astype(np.uint8)*255

    if return_all:
        return error_map,vi,vi_split,vi_merge,splitters,mergers,fm_wshed,sm_wshed,i_splits,i_merges,fm_wshed,sm_wshed,misses,extras
    return error_map
...
def VI_from_struct(fm_struct,sm_struct,rm_bounds=5):
    ...
    if rm_bounds>0:
        ker=np.ones((rm_bounds,rm_bounds))
        not_boundary=np.logical_and(skmorph.binary_erosion(fm_wshed_orig>0,ker),sm_prob<mb_thres_low)
...
def VI(fm_labels,sm_labels):
    ...
    sm_unique,sm_inverse,sm_counts=np.unique(sm_labels,return_inverse=True,return_counts=True)
    fm_unique,fm_inverse,fm_counts=np.unique(fm_labels,return_inverse=True,return_counts=True)
...
def get_error_GT(prob_map,prob_map_ref,h1=0.3,h2=0.04,error_thres=1e-5,dilation=0,use_matlab=False,max_size=200000):
    ...
    error1=np.logical_and(error1,~skmorph.binary_dilation(prob_map_hmin1_w==0,np.ones((DIL,DIL)))).astype(np.uint16)
    temp=error1>0
    error1[temp]=E1[temp]
    ...
    error2=np.logical_and(error2,~skmorph.binary_dilation(prob_map_ref_hmin2_w==0,np.ones((DIL,DIL)))).astype(np.uint16)
    if dilation==0:
...
class EM2MBNet():
    ...
    def __init__(self,params={}):
        ...
        if self.params["em2mb_net_package"]=="UNet":
            self.em2mb_net=UNet.UNet(n_channels=1,n_classes=2,**self.params["em2mb_net_params"])
        elif self.params["em2mb_net_package"]=="UNet2":
            self.em2mb_net=UNet2.UNet(n_channels=1,n_classes=2,**self.params["em2mb_net_params"])
        else:
...
class SmartEM():
    defaultparams={"em2mb_net":"../data_perm/Berghia/em2mb_net.pth",
    ...
    def __init__(self,params={}):
        ...
        assert self.params["error_net_type"] in ["cat","prob"], "Error Net Type should be cat or prob"
        ...
        if self.params["error_net_package"]=="UNet":
            self.error_net=UNet.UNet(n_channels=1,n_classes=(2 if self.params["error_net_type"]=="cat" else 1),**self.params["error_net_params"])
        elif self.params["error_net_package"]=="UNet2":
            self.error_net=UNet2.UNet(n_channels=1,n_classes=(2 if self.params["error_net_type"]=="cat" else 1),**self.params["error_net_params"])
        else:
            assert False, "not an option"
    ...
    def get_error_prob(self,mb):
        if self.params["error_net_type"]=="cat":
            ...
            error_prob=self.error_net(torch.tensor(mb/255)[None,None].to(device=self.device,dtype=torch.float32))[0,0]
    ...
    def smart_mock(self,fast_em,slow_em,rescan_map=None):
        ...
        return fast_em,slow_em,fast_mb,slow_mb,error_prob,rescan_map,fused_em,fused_mb

    def pad(self,binim):
        if self.params["pad"]==0:
            padded=binim
        else:
            padded=skmorph.binary_dilation(binim,np.ones((self.params["pad"],self.params["pad"])))
        return padded
